...
import { Transaction } from "../../../../../src/standard/transactions/format-1/Transaction.ts";
...
describe("methods", () => {
  describe("builder()", () => {
    test("can build a quantity transaction", async () => {
      ...
      const txFormat1Builder = Transaction
        .builder()
      ...
      // Create the signer for signing format 1 transactions
      const txSigner = new ArweaveJSSigner({ arweave });

      // Build a format 1 transaction
      const txFormat1 = txFormat1Builder.build();

      // Sign the format 1 transaction
      const txFormat1Signed = await txSigner.sign(txFormat1, {
        key: testSigningKey,
      });

      // As a sanity check, verify the transaction's signature data is correct
      const verified = await txSigner.verify(txFormat1Signed);

      expect(verified).toBe(true);

      const expected = {
        format: 1,
        id: txFormat1Signed.id,
        last_tx:
          "93yl0l9qof315o1p0q5hi1811kncm2rdx3o13ann8imlp3vn4pgz4l78kvvm6q54",
        owner:
        ...
        reward: "65596",
        data_tree: undefined,
        signature: txFormat1Signed.signature,
      };
    ...
    test.skip("can build a data transaction", async () => {
      ...
      const txSigner = new ArweaveJSSigner({
        arweave,
      });
      ...
      const signed = await txSigner.sign(tx, { key: testSigningKey });

      // As a sanity check, verify the transaction's signature data is correct
      const verified = await txSigner.verify(signed);
